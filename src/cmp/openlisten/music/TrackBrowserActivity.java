        ...
        implements View.OnCreateContextMenuListener, MusicUtils.Defs, ServiceConnection
{
    ...
    public void init(Cursor newCursor, boolean isLimited) {
        ...
        f.addAction(OLMediaPlaybackService.QUEUE_CHANGED);
        if ("nowplaying".equals(mPlaylist)) {
            try {
                int cur = MusicUtils.sService.getQueuePosition();
                setSelection(cur);
                registerReceiver(mNowPlayingListener, new IntentFilter(f));
                mNowPlayingListener.onReceive(this, new Intent(OLMediaPlaybackService.META_CHANGED));
            ...
            mTrackListListener.onReceive(this, new Intent(OLMediaPlaybackService.META_CHANGED));
        }
    }
    ...
    private void setTitle() {
        ...
        if (mAlbumId != null) {
            ...
            if (numresults > 0) {
                ...
                if (fancyName == null || fancyName.equals("")) {
                    fancyName = getString(R.string.unknown_album_name);
                }
            }
        } else if (mPlaylist != null) {
            if (mPlaylist.equals("nowplaying")) {
                if (MusicUtils.getCurrentShuffleMode() == OLMediaPlaybackService.SHUFFLE_AUTO) {
...
//            }
    ...
    private BroadcastReceiver mNowPlayingListener = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(OLMediaPlaybackService.META_CHANGED)) {
                getListView().invalidateViews();
            } else if (intent.getAction().equals(OLMediaPlaybackService.QUEUE_CHANGED)) {
                if (mDeletedOneRow) {
