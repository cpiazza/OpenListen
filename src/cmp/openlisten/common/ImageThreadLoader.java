...
public class ImageThreadLoader {
	...
	//private static final int maxHashSize = 35;
	...
	private void loadImage(final QueueItem item) {
		final Bitmap bmp = readBitmapFromNetwork(item.url);
		if( bmp != null ) {
			
			// Control the growth of the Cache... assume itemat spot 0 is oldest.
			/**
			if (Cache.size() >= maxHashSize) {
				Set<String> s = Cache.keySet();
				Object[] a = s.toArray();
				String st = (String)a[0];
				Cache.remove(st);
			}
			**/
